...
Public Class FormSeleniumMCP
    ...
    ' This next value should really be Long because of how it's defined in the database, however, due to VBA limitations, VBA cannot
    ' send Long values to our MosaicDBAccess DLL, so the DLL was modified to take in Integers instead.
    Private executableHistoryID As Integer = 0
...
#Region "Private subs and functions unrelated to events"
    ...
    Private Function MCP(ByVal executableHistoryID As Integer, ByVal resultsFolder As String) As Integer
        ...
        appendToLog("Connecting to RSTAR database...")
        Try
            ...
            If returnCallValues.Length > 0 AndAlso returnCallValues(0).Trim <> "0" Then
                appendToLog("Connection to RSTAR database successful.")
            Else
                Throw New Exception()
            End If
        Catch ex As Exception
            MCP = -1
            mcpMessageError = "Error connecting to RSTAR database: " & returnCall
        ...
        myScripts.LogPath = My.Computer.FileSystem.CombinePath(resultsFolder, executionLog)                         ' ...& initialize its properties.
        myScripts.ExecutableHistoryRecordID = executableHistoryID
        ...
        For Each ID As Integer In executableStepIDs
            abortRun = False
            Try
                returnRecordSet = mosaicDll.RSTAR_GetActionHistoryRecordIDsAndCreateDataFile(ID, _
                  My.Computer.FileSystem.CombinePath(resultsFolder, assignedDPExcelFileName & ID.ToString & assignedDPExcelExtension), _
                  "Deep", senderMCP)
            Catch ex As Exception
                MCP = -1
                        ...
                        Try
                            ...
                            Else
                                Select Case CType(actionReturnValue(0), Integer)
                                    Case executionStatusFailedAbort
                                        ' (*** Don't pass this new value on to the DLL for now. ***)
                                        mosaicDll.RSTAR_UpdateActionExecutionStatus(recordID, "Failed") 'do not change the Status name
                                        appendToLog("  Action FAILED. Run stopped. " & actionReturnValue(1).ToString, 2)
        ...
        'disconnect from db
        Try
            appendToLog("Disconnecting from RSTAR database...")
            mosaicDll.DisconnectFromDB()
            appendToLog("Disconnection from RSTAR database successful.")
        Catch ex As Exception
            MCP = -1
            mcpMessageError = "Error disconnecting from RSTAR database: " & ex.Message
            Exit Function
        End Try
        Try
            mosaicDll.Logger(senderMCP, "***Automation Execution " & CStr(IIf(abortRun, "Aborted", "Complete")) & "***", _
                             My.Computer.FileSystem.CombinePath(resultsFolder, executionLog))
        Catch ex As Exception
            MCP = -1
            mcpMessageError = "Error writing to " & executionLog & ": " & ex.Message
            Exit Function
